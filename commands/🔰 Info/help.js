  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        ...
        if (cmd.name) embed.addField("**<:arrow:> Command name**", `\`${cmd.name}\``);
        if (cmd.name) embed.setTitle(`<:arrow:> Detailed Information about: \`${cmd.name}\``);
        if (cmd.description) embed.addField("**<:arrow:> Description**", `\`\`\`${cmd.description}\`\`\``);
        if (cmd.aliases) try {
          embed.addField("**<:arrow:> Aliases**", `\`${cmd.aliases.map((a) => `${a}`).join("`, `")}\``);
        } catch { }
        if (cmd.cooldown) embed.addField("**<:arrow:> Cooldown**", `\`\`\`${cmd.cooldown} Seconds\`\`\``);
        else embed.addField("**<:arrow:> Cooldown**", `\`\`\`3 Seconds\`\`\``);
        if (cmd.usage) {
          embed.addField("**<:arrow:> Usage**", `\`\`\`${config.prefix}${cmd.usage}\`\`\``);
          embed.setFooter("Syntax: <> = required, [] = optional", es.footericon);
        }
        if (cmd.useage) {
          embed.addField("**<:arrow:> Useage**", `\`\`\`${config.prefix}${cmd.useage}\`\`\``);
          embed.setFooter("Syntax: <> = required, [] = optional", es.footericon);
        }
        return message.channel.send(embed);
      } else {
        ...
        let button_dc = new MessageButton().setStyle('url').setLabel('Get your Own Bot').setURL("https://limsathya.netlify.app/")
        let button_invite = new MessageButton().setStyle('url').setLabel('Invite Public Version').setURL("https://discord.com/api/oauth2/authorize?client_id=861249452789596200&permissions=8&scope=bot")
        ...
        let FIRSTEMBED = new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter("Page Home\n" + client.user.username + " | Limsathya ", client.user.displayAvatarURL())
...
📶 **\`${Math.floor(client.ws.ping)}ms\` Ping**`)
        ...
        let d_button_dc = new MessageButton().setStyle('url').setLabel('Support server').setURL("https://discord.com/uYRDDx7yFn");
        let d_button_invite = new MessageButton().setStyle('url').setLabel('Invite Public Version').setURL("https://discord.com/api/oauth2/authorize?client_id=861249452789596200&permissions=8&scope=bot");
      ...
      function allotherembeds() {
        ...
        var embed3 = new MessageEmbed()
          ...
          .setFooter(`Page 4 / 6 | Limsathya \nTo see command Descriptions and Information, type: ${config.prefix}help [CMD NAME]`, client.user.displayAvatarURL());
        embeds.push(embed3)
        var embed4 = new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setTitle(`⚙️ Settings`)
          .setDescription(`⚙️ **Settings**\n> ${client.commands.filter((cmd) => cmd.category === "⚙️ Settings").map((cmd) => `\`${cmd.name}\``).join(", ")}`)
          .setFooter(`Page 5 / 6 | Limsathya  \nTo see command Descriptions and Information, type: ${config.prefix}help [CMD NAME]`, client.user.displayAvatarURL());
        embeds.push(embed4)
        var embed5 = new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setTitle("Bot Creator")

          .addField(`> ***[XG Official](https://discord.gg/uYRDDx7yFn)***\n>`)
          .addField(`**__Bot Creator Information__**`, `>>> 💯 This Bot has been made by Limsathya :\n[Discord](https://discord.com/uYRDDx7yFn)`)
          .setFooter(`Page 6 / 6 | Limsathya  \nTo see command Descriptions and Information, type: ${config.prefix}help [CMD NAME]`, client.user.displayAvatarURL());
        embeds.push(embed5)
        return embeds
      }
    } catch (e) {
